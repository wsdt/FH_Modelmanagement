...
(function(Context, namespace, hydratedCssClass, resourcesUrl, s) {
  "use strict";
  s = document.querySelector("script[data-namespace='fhkuf']");
  if (s) {
  ...
  (function(window, document, Context, namespace) {
    ...
    function initStyleTemplate(
      ...
      if (style) {
        ...
        const styleModeId =
          cmpMeta.tagNameMeta + (styleMode || DEFAULT_STYLE_MODE);
        if (!cmpMeta[styleModeId]) {
          false;
          {
            ...
            true;
            {
              ...
              const styleContent = [
                "<style",
                ` data-style-tag="${cmpMeta.tagNameMeta}"`
              ];
              domApi.$setAttribute(
                templateElm,
                "data-tmpl-style-tag",
              ...
              if (styleMode) {
                ...
                domApi.$setAttribute(
                  templateElm,
                  "data-tmpl-style-mode",
                  styleMode
                );
              }
              if (
                2 /* ScopedCss */ === encapsulation ||
                (1 /* ShadowDom */ === encapsulation &&
                  !domApi.$supportsShadowDom)
              ) {
                styleContent.push(' data-style-scoped="true"');
                domApi.$setAttribute(
                  templateElm,
                  "data-tmpl-style-scoped",
                  "true"
                );
              }
              styleContent.push(">");
    ...
    function attachStyles(plt, domApi, cmpMeta, hostElm) {
      ...
      const shouldScopeCss =
        2 /* ScopedCss */ === cmpMeta.encapsulationMeta ||
        (1 /* ShadowDom */ === cmpMeta.encapsulationMeta &&
          !plt.domApi.$supportsShadowDom);
      ...
      if (styleTemplate) {
        ...
        if ((true, domApi.$supportsShadowDom))
          if (1 /* ShadowDom */ === cmpMeta.encapsulationMeta)
        ...
        appliedStyles ||
          plt.componentAppliedStyles.set(
            styleContainerNode,
            (appliedStyles = {})
          );
        ...
        if (!appliedStyles[styleId]) {
          ...
          false;
          {
            ...
            const dataStyles = styleContainerNode.querySelectorAll(
              "[data-styles]"
            );
            domApi.$insertBefore(
              styleContainerNode,
              styleElm,
              (dataStyles.length &&
                dataStyles[dataStyles.length - 1].nextSibling) ||
                styleContainerNode.firstChild
            );
          }
        }
      }
    ...
    function createDomApi(App, win, doc) {
      ...
      const domApi = {
        ...
        $insertBefore: (parentNode, childNode, referenceNode) =>
        ...
        $setAttributeNS: (elm, namespaceURI, qualifiedName, val) =>
          ...
          assignersUnregListeners &&
            ...
            (eventName
              ...
              : // remove all event listeners
                Object.keys(assignersUnregListeners).forEach(
                  assignersEventName => {
                    assignersUnregListeners[assignersEventName] &&
                      assignersUnregListeners[assignersEventName]();
                  }
        ...
        $parentElement: (elm, parentNode) =>
          ...
          11 /* DocumentFragment */ === domApi.$nodeType(parentNode)
            ? parentNode.host
            : parentNode
      };
      true;
    ...
    function updateAttribute(
      ...
      memberName,
      newValue,
      isBooleanAttr = "boolean" === typeof newValue
    ) {
      const isXlinkNs =
        memberName !== (memberName = memberName.replace(/^xlink\:?/, ""));
      if (
        null == newValue ||
        (isBooleanAttr && (!newValue || "false" === newValue))
      )
        isXlinkNs
        ...
        if ("style" === memberName) {
          ...
          if (
            ("list" !== memberName &&
              ...
              !isSvg &&
              (memberName in elm ||
                (-1 !== ["object", "function"].indexOf(typeof newValue) &&
                  null !== newValue))) ||
            false
          ) {
            // Properties
            ...
            null != newValue && "key" !== memberName
              ...
              : (isSvg ||
                  (plt.domApi.$hasAttribute(elm, memberName) &&
                    (null == newValue || false === newValue))) &&
          ...
          memberName =
            toLowerCase(memberName) in elm
              ? toLowerCase(memberName.substring(2))
              : toLowerCase(memberName[2]) + memberName.substring(3);
          newValue
            ? newValue !== oldValue &&
              // add listener
    ...
    function updateElement(plt, oldVnode, newVnode, isSvgMode, memberName) {
      ...
      const elm =
        11 /* DocumentFragment */ === newVnode.elm.nodeType && newVnode.elm.host
          ? newVnode.elm.host
          : newVnode.elm;
      ...
      for (memberName in oldVnodeAttrs)
        (newVnodeAttrs && null != newVnodeAttrs[memberName]) ||
          null == oldVnodeAttrs[memberName] ||
          setAccessor(
            plt,
            elm,
            memberName,
      ...
      for (memberName in newVnodeAttrs)
        (memberName in oldVnodeAttrs &&
          newVnodeAttrs[memberName] ===
            ("value" === memberName || "checked" === memberName
              ? elm[memberName]
              : oldVnodeAttrs[memberName])) ||
          setAccessor(
            plt,
    ...
    function createRendererPatch(plt, domApi) {
      ...
      function createElm(
        ...
        if (isDef(newVNode.vtext))
          ...
          // create element
          elm = newVNode.elm = (true,
          isSvgMode || "svg" === newVNode.vtag
            ? domApi.$createElementNS(
                "http://www.w3.org/2000/svg",
                newVNode.vtag
              )
            : domApi.$createElement(
                (true, newVNode.isSlotFallback ? "slot-fb" : newVNode.vtag)
              ));
          plt.isDefinedComponent(elm) && plt.isCmpReady.delete(hostElm);
          true;
          isSvgMode =
            "svg" === newVNode.vtag ||
            ("foreignObject" !== newVNode.vtag && isSvgMode);
        ...
        if (newVNode.isSlotFallback || newVNode.isSlotReference) {
          ...
          oldVNode =
            oldParentVNode &&
            oldParentVNode.vchildren &&
            oldParentVNode.vchildren[childIndex];
          oldVNode &&
            oldVNode.vtag === newVNode.vtag &&
            oldParentVNode.elm &&
      ...
      function addVnodes(
        ...
        containerElm =
          (contentRef && domApi.$parentNode(contentRef)) || parentElm;
        containerElm.shadowRoot &&
          domApi.$tagName(containerElm) === hostTagName &&
      ...
      function updateChildren(
        ...
        while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx)
          if (null == oldStartVnode)
          ...
          else if (null == newStartVnode) newStartVnode = newCh[++newStartIdx];
            ...
            // Vnode moved right
            ("slot" !== oldStartVnode.vtag && "slot" !== newEndVnode.vtag) ||
              putBackInOriginalLocation(domApi.$parentNode(oldStartVnode.elm));
            patchVNode(oldStartVnode, newEndVnode);
            domApi.$insertBefore(
              parentElm,
              oldStartVnode.elm,
              domApi.$nextSibling(oldEndVnode.elm)
            );
            oldStartVnode = oldCh[++oldStartIdx];
            newEndVnode = newCh[--newEndIdx];
            ...
            for (i = oldStartIdx; i <= oldEndIdx; ++i)
              if (
                oldCh[i] &&
                isDef(oldCh[i].vkey) &&
                oldCh[i].vkey === newStartVnode.vkey
              ) {
                idxInOld = i;
                break;
              }
      ...
      function patchVNode(oldVNode, newVNode, defaultHolder) {
        ...
        isSvgMode =
          ...
          void 0 !== newVNode.elm.ownerSVGElement;
        isSvgMode =
          "svg" === newVNode.vtag ||
          ("foreignObject" !== newVNode.vtag && isSvgMode);
        if (isDef(newVNode.vtext))
            ...
            (defaultHolder = elm["s-cr"])
              ? // this element has slotted content
                domApi.$setTextContent(
                  domApi.$parentNode(defaultHolder),
                  newVNode.vtext
                )
              ...
              for (j = 0; j < ilen; j++)
                if (childNodes[j]["s-hn"] !== childNode["s-hn"]) {
                  ...
                  if ("" !== slotNameAttr) {
                    ...
                    if (
                      1 /* ElementNode */ === nodeType &&
                      slotNameAttr ===
                        domApi.$getAttribute(childNodes[j], "slot")
                    ) {
                  ...
                  // should hide the default fallback slot node
                    ...
                    1 /* ElementNode */ === nodeType ||
                    (3 /* TextNode */ === nodeType &&
                      "" !== domApi.$getTextContent(childNodes[j]).trim())
                  ) {
                    childNode.hidden = true;
                    break;
                  }
          ...
          if (childNode["s-sr"] && (node = childNode["s-cr"])) {
            ...
            for (j = hostContentNodes.length - 1; j >= 0; j--) {
              ...
              if (
                ...
                if (
                  (((3 /* TextNode */ === nodeType ||
                    8 /* CommentNode */ === nodeType) &&
                    "" === slotNameAttr) ||
                    (1 /* ElementNode */ === nodeType &&
                      null === domApi.$getAttribute(node, "slot") &&
                      "" === slotNameAttr) ||
                    (1 /* ElementNode */ === nodeType &&
                      domApi.$getAttribute(node, "slot") === slotNameAttr)) &&
          ...
          for (i = 0; i < relocateNodes.length; i++) {
            ...
            while ((orgLocationNode = domApi.$previousSibling(orgLocationNode)))
              if (
                ...
                parentNodeRef === domApi.$parentNode(refNode) &&
                (refNode = domApi.$nextSibling(refNode)) &&
                refNode &&
                !refNode["s-nr"]
              ) {
            ...
            if (
              ((!insertBeforeNode &&
                parentNodeRef !==
                  domApi.$parentNode(relocateNode.nodeToRelocate)) ||
                domApi.$nextSibling(relocateNode.nodeToRelocate) !==
                  insertBeforeNode) &&
    ...
    function callNodeRefs(vNode, isDestroy) {
      if (vNode) {
        vNode.vattrs &&
          vNode.vattrs.ref &&
          vNode.vattrs.ref(isDestroy ? null : vNode.elm);
        vNode.vchildren &&
    ...
    function addChildSsrVNodes(
      ...
      if (checkNestedElements && 1 /* ElementNode */ === nodeType) {
        ...
        8 /* CommentNode */ === domApi.$nodeType(previousComment)
      ) {
        // split the start comment's data with a period
    ...
    function createQueueClient(App, win) {
      ...
      function flush() {
        ...
        (rafPending =
          domReads.length + domWrites.length + domWritesLow.length > 0)
    ...
    function generateDevInspector(App, namespace, win, plt) {
      ...
      devInspector.getInstance ||
        (devInspector.getInstance = elm => {
          return Promise.all(
            devInspector.apps.map(app => {
    ...
    function getMembersMeta(properties) {
      return Object.keys(properties).reduce(
        (membersMap, memberKey) => {
          ...
          } else if (prop.elementRef) category = "elements";
          else if (prop.method) category = "methods";
          else {
            category = "props";
            let type = "any";
            if (prop.type) {
        ...
        if (child && void 0 !== child.pop)
          ...
          (simple = "function" !== typeof nodeName) &&
            (null == child
              ...
              : "number" === typeof child
              ? (child = String(child))
              : "string" !== typeof child && (simple = false));
          simple && lastSimple
    ...
    const utils = {
      forEach: (children, cb) => {
        children.forEach((item, index, array) =>
          cb(VNodeToChild(item), index, array)
        );
      },
      map: (children, cb) => {
        return children.map((item, index, array) =>
          childToVNode(cb(VNodeToChild(item), index, array))
        );
      }
    };
    function initCoreComponentOnReady(
      plt,
      App,
      win,
      apps,
      ...
      if (queuedComponentOnReadys) {
        ...
        for (i = queuedComponentOnReadys.length - 1; i >= 0; i--)
          ...
          App.componentOnReady(
            queuedComponentOnReadys[i][0],
            queuedComponentOnReadys[i][1]
          ) &&
    ...
    function initHostSnapshot(
      ...
      if (
        !hostElm["s-cr"] &&
        !domApi.$getAttribute(hostElm, SSR_VNODE_ID) &&
        (!domApi.$supportsShadowDom ||
          1 /* ShadowDom */ !== cmpMeta.encapsulationMeta)
      ) {
      ...
      cmpMeta.membersMeta &&
        Object.keys(cmpMeta.membersMeta).forEach(memberName => {
          (attribName = cmpMeta.membersMeta[memberName].attribName) &&
            (hostSnapshot.$attributes[attribName] = domApi.$getAttribute(
              hostElm,
              attribName
            ));
        });
    ...
    function registerWithParentComponent(plt, elm, ancestorHostElement) {
      ...
      while (
        (ancestorHostElement = plt.domApi.$parentElement(ancestorHostElement))
      )
        ...
        if (plt.isDefinedComponent(ancestorHostElement)) {
          ...
          if (!plt.isCmpReady.has(elm)) {
            ...
            // ensure there is an array to contain a reference to each of the child elements
            // and set this element as one of the ancestor's child elements it should wait on
            (ancestorHostElement["s-ld"] =
              ancestorHostElement["s-ld"] || []).push(elm);
          }
          break;
        }
    }
    function parseComponentLoader(cmpRegistryData) {
      ...
      return {
        ...
        // parse listener meta
        listenersMeta: listenerMeta
          ? listenerMeta.map(parseListenerData)
          : void 0
      };
    }
    ...
    function render(plt, cmpMeta, hostElm, instance) {
      try {
        ...
        const useNativeShadowDom =
          "shadow" === encapsulation && plt.domApi.$supportsShadowDom;
        let reflectHostAttr;
        let rootElm = hostElm;
        false;
      ...
      if (!plt.isDisconnectedMap.has(elm)) {
        ...
        if (isInitialLoad) {
          ...
          if (ancestorHostElement && !ancestorHostElement["s-rn"]) {
            ...
            (ancestorHostElement["s-rc"] =
              ancestorHostElement["s-rc"] || []).push(() => {
          ...
          instance = initComponentInstance(
            plt,
            elm,
            plt.hostSnapshotMap.get(elm)
          );
          false;
        } else false;
        // if this component has a render function, let's fire
    ...
    function defineMember(
      ...
      function setComponentProp(newValue, elm) {
        ...
        if (elm)
          if (property.state || property.mutable)
            ...
            true;
            console.warn(
              `@Prop() "${memberName}" on "${elm.tagName}" cannot be modified.`
            );
          }
      }
      if (property.type || property.state) {
        ...
        if (!property.state) {
          !property.attr ||
            (void 0 !== values[memberName] && "" !== values[memberName]) ||
            // check the prop value from the host element attribute
            ((hostAttributes = hostSnapshot && hostSnapshot.$attributes) &&
              isDef((hostAttrValue = hostAttributes[property.attr])) &&
              // looks like we've got an attribute value
              // let's set it to our internal values
              (values[memberName] = parsePropertyValue(
                property.type,
                hostAttrValue
              )));
          true;
        ...
        property.watchCallbacks &&
          (values[
            WATCH_CB_PREFIX + memberName
          ] = property.watchCallbacks.slice());
    ...
    function proxyComponentInstance(
      plt,
      cmpConstructor,
      elm,
      instance,
    ...
    function initComponentInstance(
      plt,
      elm,
      hostSnapshot,
      instance,
    ...
    function propagateComponentReady(
      plt,
      elm,
      index,
      ...
      if ((ancestorHostElement = plt.ancestorHostElementMap.get(elm))) {
        ...
        if (ancestorsActivelyLoadingChildren) {
          ...
          ancestorsActivelyLoadingChildren.length ||
    ...
    function hmrFinish(plt, cmpMeta, elm) {
      if (!plt.hasListenersMap.has(elm)) {
        ...
        if (
          ...
          cmpMeta.componentConstructor.listeners
        ) {
          cmpMeta.listenersMeta = cmpMeta.componentConstructor.listeners.map(
            lstn => {
              const listenerMeta = {
            ...
            definePropertyGetterSetter(
              ...
              function setHostElementProp(newValue) {
                ...
                setValue(
                  ...
                  this,
                  memberName,
                  parsePropertyValue(member.propType, newValue)
                );
              }
            )
    ...
    function initHostElement(
      ...
      if (cmpMeta.membersMeta) {
        ...
        true;
        {
          ...
          HostElementConstructor.attributeChangedCallback = function(
            attribName,
            _oldVal,
            newVal
          ) {
    ...
    function proxyProp(domApi, controllerComponents, ctrlTag, proxyMethodName) {
      return function() {
        ...
        return loadComponent(domApi, controllerComponents, ctrlTag).then(
          ctrlElm => ctrlElm[proxyMethodName].apply(ctrlElm, args)
        );
      };
    }
    function loadComponent(domApi, controllerComponents, ctrlTag) {
      ...
      if (body) {
        ...
        if (!ctrlElm) {
          ctrlElm = controllerComponents[ctrlTag] = domApi.$createElement(
            ctrlTag
          );
    ...
    function createPlatformMain(
      ...
      const plt = {
        ...
        isClient: true,
        isDefinedComponent: elm =>
          !!(globalDefined[domApi.$tagName(elm)] || plt.getComponentMeta(elm)),
        nextId: () => namespace + ids++,
        onError: (err, type, elm) =>
          console.error(err, type, elm && elm.tagName),
        propConnect: ctrlTag =>
          proxyController(domApi, controllerComponents, ctrlTag),
      ...
      App.onReady = () =>
        new Promise(resolve =>
          plt.queue.write(() =>
            plt.processingCmp.size
              ? plt.onAppReadyCallbacks.push(resolve)
              : resolve()
          )
        );
      ...
      function defineComponent(cmpMeta, HostElementConstructor) {
        ...
        if (!win.customElements.get(tagNameMeta)) {
          ...
          HostElementConstructor.observedAttributes = Object.values(
            cmpMeta.membersMeta
          )
      ...
      function requestBundle(cmpMeta, elm, hmrVersionId) {
        if (cmpMeta.componentConstructor)
          ...
          true;
          {
            ...
            const bundleId =
              "string" === typeof cmpMeta.bundleIds
                ? cmpMeta.bundleIds
            ...
            import(url).then(
              importedModule => {
                ...
                try {
                  ...
                  cmpMeta.componentConstructor =
                    importedModule[dashToPascalCase(cmpMeta.tagNameMeta)];
                  initStyleTemplate(
                    domApi,
                    cmpMeta,
      ...
      (App.components || [])
        ...
        .forEach(cmpMeta =>
          defineComponent(cmpMeta, class extends HTMLElement {})
        );
