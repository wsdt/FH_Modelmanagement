        ...
        * No method must be provided for an uri. */
        ...
        let evaluate_route = function (method_name, method_func) {
            switch (method_name.toLowerCase()) {
                case "post": express_app.post(key,method_func); break;
                case "get": express_app.get(key,method_func);break;
                case "delete": express_app.delete(key,method_func);break;
                case "put": express_app.put(key,method_func);break;
                default: console.error("app:setup_routes: HTTP-method not found -> "+method_name.toLowerCase());
            }
        };
